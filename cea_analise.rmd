...
library(glmnet)
set.seed(123) 
x <-subset(data, select = c("Lácteos", "Pescado", "Suinocultura", "Avicultura de Postura", "Avicultura de Corte", "Bovinocultura"))

x = model.matrix(Bovinocultura~.,x)[,-1]
y = data$Bovinocultura
...
cv.ridge$lambda.min
cv.ridge$lambda.1se
coef(cv.ridge, cv.lasso$lambda.min)
coef(cv.ridge, cv.lasso$lambda.1se)
```
